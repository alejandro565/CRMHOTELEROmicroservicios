...
async function connectRabbitMQ(retries = 5, delay = 4000) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      conn.on('error', (err) => { console.error('[RabbitMQ] error:', err.message); channel = null; });
      conn.on('close', () => {
      ...
      console.log('[RabbitMQ] guest-service connected');
      return;
    } catch (err) {
...
function publishEvent(routingKey, payload) {
  if (!channel) { console.error('[RabbitMQ] channel not ready — dropped:', routingKey); return; }
  channel.publish(
    EXCHANGE, routingKey,
