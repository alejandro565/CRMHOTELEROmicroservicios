...
async function createHotel({ owner_id, owner_email, name, tax_id }) {
  ...
  const taxDuplicate = await Tenant.findOne({ where: { tax_id: String(tax_id) } });
  if (taxDuplicate) {
...
async function updateTenant(tenant_id, owner_id, { name, tax_id }) {
  const tenant = await _getActiveTenant(tenant_id);

  // Only the owner can update their own hotel
  if (tenant.owner_id !== owner_id) {
    throw new AppError('No tienes permiso para modificar este hotel', 403, 'FORBIDDEN');
  }

  if (tax_id && tax_id !== tenant.tax_id) {
    const taxDuplicate = await Tenant.findOne({ where: { tax_id: String(tax_id), id: { [Op.ne]: tenant_id } } });
    if (taxDuplicate) {
      throw new AppError(`El NIT "${tax_id}" ya está registrado`, 409, 'TAX_ID_DUPLICATE');
    }
  }
  ...
  await tenant.update({ 
    ...
    tax_id: tax_id ? String(tax_id) : tenant.tax_id
  });
...
module.exports = {
  ...
  createFirstHotel, createHotel, updateTenant,
