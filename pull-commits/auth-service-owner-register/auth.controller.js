...
async function login(req, res, next) {
  try {
    ...
    res.json({ ...response, token: result.accessToken });
  } catch (err) { next(err); }
}
...
async function switchTenant(req, res, next) {
  try {
    ...
    // Validate the selection token from the Authorization header
    ...
    // Must be a selection token
    if (payload.purpose !== 'hotel_selection') {
      return next(new AppError('Token no válido para esta operación', 403, 'WRONG_TOKEN_PURPOSE'));
    }

    const result = await authService.switchTenant(payload.userId, tenant_id);
...
async function registerOwner(req, res, next) {
  try {
    ...
    const { refreshToken: _rt, ...response } = result;
    res.status(201).json({ ...response, token: result.accessToken, registered: true });
  } catch (err) { next(err); }
}
